...
export default function AddListingForm({ onClose, onCreated }) {
  ...
  const onSubmit = async (e) => {
    ...
    try {
      ...
      // PostHog: Track successful listing creation
      ...
      // PostHog: Track listing creation error
      posthog.capture('listing_create_error', {
        listing_title: title,
        error_message: err?.message || 'Failed to create listing',
      });
      ...
      <div className="relative bg-white rounded-2xl p-6 w-full max-w-2xl z-10 shadow-lg">
        ...
        <form onSubmit={onSubmit} className="space-y-4">
          ...
          <div className="flex items-center gap-3 mt-4">
            <button type="submit" disabled={saving} className="px-5 py-2 rounded-full bg-green-500 text-white">{saving ? 'Saving…' : 'Create'}</button>
